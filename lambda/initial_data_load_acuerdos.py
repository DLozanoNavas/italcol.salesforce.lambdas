...
def update_existing_salesforce_records(url: str, auth_data: dict):
  ...
  existing_records = [
    {
      'Id': record['Id'],
      'ac_Cuenta__c': record['ac_Cuenta__c'],
      field_names["ic"]: record["Id_AC_ERP_balanceados__c"]
  ...
  for record in existing_records:
    account_id = record['ac_Cuenta__c']
    store_id = record['Id']
...
def post_new_salesforce_records(url: str, auth_data: dict, data: list[dict]):
    ...
    valid_data = []
    for body in data:
        id = body.get("Id_AC_ERP_balanceados__c")
        ...
        if not indexed_records.get(third_party_nit):
            ...
            body["ac_cuenta__c"] = third_party_sf_id
    ...
    for idx in range(0, total, batch_size):
        ...
        payload = {
            "records": [
                {
                    "attributes": {"type": "Acuerdo_Comercial__c", "referenceId": f"ref{index}"},
                    **record
...
if __name__ == "__main__":
    ...
    mappedData = mapper.map_acuerdos_from_view(companies_dict[selected_company].get("company"), data)
    ...
    get_url = f"{auth_result['base_url']}{config['API_URIS'].get('QUERY').get('ACUERDOS')}"
    post_url = f"{auth_result['base_url']}{config['API_URIS'].get('POST').get('ACUERDOS')}"
